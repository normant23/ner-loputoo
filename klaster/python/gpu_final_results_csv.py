...
def results(df, model_name, full_result, result_by_tags):
    # Add overall metrics
    row = {
        'model': model_name,
        'tag': 'Overall',
        'precision': round(full_result['strict']['precision'], 4),
        'recall': round(full_result['strict']['recall'], 4),
        'f1': round(full_result['strict']['f1'], 4)
    }
    ...
    for tag, stats in result_by_tags.items():
        row = {
            'model': model_name,
            'tag': tag,
            'precision': round(stats['strict']['precision'], 4),
            'recall': round(stats['strict']['recall'], 4),
            'f1': round(stats['strict']['f1'], 4)
        }
        df = pd.concat([df, pd.DataFrame([row])], ignore_index=True)
    
    return df

results_df = pd.DataFrame(columns=['model', 'tag', 'precision', 'recall', 'f1'])
input_folder = 'data/processed_gold'
#test_file = '1935-09-30_manual_annotated.json'

final_model_dir = sys.argv[2]
model_folder = f'final_model/{final_model_dir}/pre_clean_fixed/'
...
for model_name in sorted(os.listdir(model_folder)):
    ...
    results_df = results(results_df, model_name, results2, results_per_tag2)
...
results_df = results(results_df, 'estbertner', results1, results_per_tag1)
...
results_df = results(results_df, 'opetaja', results1, results_per_tag1)

#tudeng fixed
checkpoint_dir = sorted(os.listdir('final_model/model_tudeng_fixed_new'))[-1]
new2_ner = EstBERTNERTagger(model_location='final_model/model_tudeng_fixed_new/' + checkpoint_dir, output_layer='new2_ner', batch_size=batch_size)
new2 = []
for test in test_files:
    with open(os.path.join(input_folder, test), "r", encoding="utf-8") as file:
        content = file.read()
...
results1, results_per_tag1, result_indices1, result_indices_by_tag1 = new2_eval.evaluate()
print('tudeng')
results_df = results(results_df, 'tudeng', results1, results_per_tag1)

os.makedirs('final_results/pre_clean_new', exist_ok=True)
output_file = f'final_results/pre_clean_new/results_{final_model_dir[6:]}_fixed.csv'
