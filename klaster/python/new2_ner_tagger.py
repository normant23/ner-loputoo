...
def ner_decorator(text_obj, words_phrase, corresponding_bert_tokens):
    ...
    probs = [t.annotations[0]['prob'] for t in corresponding_bert_tokens]
    avg_score = round((sum(probs) / len(probs) if probs else 0.0), 4)
    #warnings.warn(f"decorator: {list(probs)}, type is {type(probs[0])}, float type is {type(float(probs[0]))}", UserWarning)
    return {"nertag": nertags[0], "prob": avg_score}


class New2NerTagger(MultiLayerTagger):
    """EstNLTK wrapper for the huggingface EstBERTNER models."""
    conf_param = ('model_location', 'bertner', 'id2label', 'tokenizer', 'custom_words_layer', 'batch_size', 
    ...
    def __init__(self, model_location: str = None, output_layer: str = 'estbertner', 
        ...
        self.bertner = AutoModelForTokenClassification.from_pretrained(self.model_location)
        ...
        self.output_layers_to_attributes = {self.output_layers[0]: ["nertag", "prob"]}
    ...
    def _get_bert_morph_tagging_label_predictions(self, input_str:str):
        """
        ...
        Args:
            ...
            top_n (int): Number of top predictions to return for each token. (deleted)

        Returns:
            List[dict]: Each token's top N predictions with their probabilities.
        """
        # Tokenize the input string
        tokens, batch_encoding = self.tokenize_with_bert(input_str)
        ...
        self.bertner.to(device)
        token_indexes = token_indexes.to(device)

        # Get predictions
        with torch.no_grad():
            output = self.bertner(token_indexes)

        #output = output.to('cpu')

        logits = output.logits.to('cpu').squeeze()
        ...
        for i, token_data in enumerate(tokens):
            ...
            top_n_indices = torch.topk(token_probs, 1).indices  # Top N label indices
            ...
            return_probs.append(top_n_probs[0])
            return_labels.append(top_n_labels[0])
        ...
        return tokens, return_probs, return_labels
    ...
    def _make_layers(self, text: Text, layers: MutableMapping[str, Layer], status: dict) -> Layer:
        ...
        prob_attr = self.output_layers_to_attributes[self.output_layers[0]][1]
        ...
        for text_chunk, (chunk_start, chunk_end) in zip(text_chunks, text_indexes):
            ...
            bert_tokens = self.tokenize_with_bert(text_chunk, include_spanless=True)[0]
            ...
            tokens, scores, labels = self._get_bert_morph_tagging_label_predictions(text_chunk)
            # C) Combine NE labels with Bert tokens to get full NE annotations
            entity_spans = []
            entity_scores = []
            entity_type = None
            for span, label, score in zip(tokens, labels, scores):
                if span[0] is None and span[1] is None:
                    # Skip special tokens (e.g. [CLS], [SEP])
                    continue                
                corrected_start = chunk_start + span[0]
                corrected_end   = chunk_start + span[1]
                nerlayer.add_annotation(EnvelopingBaseSpan([ElementaryBaseSpan(corrected_start, corrected_end)]), {nertag_attr: label, prob_attr: score})
