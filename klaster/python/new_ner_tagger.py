...
def ner_decorator(text_obj, words_phrase, corresponding_bert_tokens):
    ...
    return {"nertag": nertags[0], "prob": round(float(probs[0]), 4)}


class NewNerTagger(MultiLayerTagger):
    ...
    def _make_layers(self, text: Text, layers: MutableMapping[str, Layer], status: dict) -> Layer:
        ...
        for text_chunk, (chunk_start, chunk_end) in zip(text_chunks, text_indexes):
            ...
            for detected_entity in response:
                ...
                scores[detected_entity['index']] = detected_entity['score']
            # C) Combine NE labels with Bert tokens to get full NE annotations
            entity_spans = []
            entity_scores = []
            entity_type = None
            for span, label, score in zip(bert_tokens, labels, scores):
                ...
                if label == "O":
                    if entity_spans:
                        #avg_score = sum(entity_scores) / len(entity_scores) if entity_scores else 0.0
                        #warnings.warn(f"score: list is {entity_scores}", UserWarning)
                        nerlayer.add_annotation(EnvelopingBaseSpan(entity_spans),
                                                **{nertag_attr: entity_type, prob_attr: entity_scores[0]})
                        entity_spans = []
                        entity_scores = []
                    continue
