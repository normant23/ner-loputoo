...
class FirstNerTagger(Retagger):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.conf_param = ('model_location', 'get_top_n_predictions', 'bert_tokenizer', 'bert_morph_tagging', 'id2label', \
                           'token_level', 'disambiguate', 'sentences_layer', 'words_layer', 'output_layer', \
        ...
        self.output_attributes = ['bert_tokens', 'ner_label', 'probability']
        if self.token_level:
    ...
    def _make_layer(self, text: Text, layers: MutableMapping[str, Layer], status: dict) -> Layer:
        ...
        first_ner_layer = Layer(name=self.output_layer, 
            ...
            for sent_chunk, (chunk_start, chunk_end) in zip(sent_chunks, sent_chunk_indexes):
                ...
                for token_data in top_n_predictions:
                    ...
                    for label, prob in zip(all_labels, all_probabilities):
                        ...
                        first_ner_layer.add_annotation(token_span, **annotation)

        # Add annotations
        if self.token_level:
            # Return token level annotations
            return first_ner_layer
            ...
            first_ner_layer = self._bert_tokens_rewriter.make_layer(text, layers={first_ner_layer.name: first_ner_layer})

        assert len(first_ner_layer) == len(words_layer), \
        f"Failed to rewrite '{first_ner_layer.name}' layer tokens to '{words_layer.name}' layer words: {len(first_ner_layer)} != {len(words_layer)}"
    ...
    def _change_layer(self, text, layers, status=None):
        ...
        if self.token_level:
            raise Exception( ('(!) Cannot use BertMorphTagger as a disambiguator if '+\
                              'token_level==True.') )
        # Validate inputs
        first_ner_layer = layers[self.output_layer]
        for attr in ['partofspeech', 'form']:
            if attr not in first_ner_layer.attributes:
                raise Exception( ('(!) Missing attribute {!r} in output_layer {!r}.'+\
                                  '').format(attr, first_ner_layer.name) )
        # Create disambiguation layer
        disamb_layer = self._make_layer(text, layers, status)
        # Disambiguate input_morph_analysis_layer
        assert len(first_ner_layer) == len(disamb_layer)
        for original_word, disamb_word in zip(first_ner_layer, disamb_layer):
...
def rewriter_decorator_BERT(text_obj, word_index, span):
    ...
    Aggregates the <code>ner_labels</code> and <code>probabilities</code> from <code>shared_bert_tokens</code>, finds the most
    ...
    for sp in span:
        top_1_label = sp['ner_label'][0] # Get top-1 label
    ...
    for sp in span:
        if most_frequent_label in sp['ner_label']:
            ...
            #raise ValueError(str(annotations))
            return annotations
